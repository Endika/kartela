...
import { CATEGORIES, LANGS } from '../src/domain/film'
...
describe('catalog', () => {
  it('ships at least 140 films', () => {
    expect(FILMS.length).toBeGreaterThanOrEqual(140)
  })

  it('has films in every category', () => {
    for (const category of CATEGORIES) {
      const films = FILMS.filter((film) => film.category === category)
      expect(films.length, category).toBeGreaterThan(0)
    }
  })

  it('uses no category outside the four known ones', () => {
    const unknown = FILMS.filter((film) => !CATEGORIES.includes(film.category))
  ...
  it('gives every film a plausible release year', () => {
    const odd = FILMS.filter((film) => film.year < 1937 || film.year > 2026)
  ...
  it('ships a poster file for every film', () => {
    ...
    const missing = FILMS.filter((film) => !onDisk.has(film.poster))
    expect(missing.map((film) => film.poster)).toEqual([])
  })

  it('leaves no orphan poster behind', () => {
    const referenced = new Set(FILMS.map((film) => film.poster))
